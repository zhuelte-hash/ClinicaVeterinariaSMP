  ...
  template: `
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      @if (cart.items().length === 0) { <div class="text-center py-12 bg-white rounded-2xl border">
        ...
        <p class="rounded-2xl bg-sky-50 p-4 text-center text-sm font-semibold text-[#0B1B6D]">Carrito demostrativo. No se realizan pagos ni pedidos desde este sitio.</p>
      </div> }
    </div>
  `,
})
