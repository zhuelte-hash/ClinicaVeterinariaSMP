...
@Component({
  ...
  template: `
    <section class="mx-auto max-w-[1500px]">
      <header class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><p class="text-xs font-black uppercase tracking-[.18em] text-teal-600">Venta presencial</p><h2 class="mt-2 text-3xl font-black text-[#102a43]">Punto de venta</h2><p class="mt-2 text-sm text-slate-500">Cobra productos y servicios en una sola boleta.</p></div>
      ...
      <div class="grid gap-6 xl:grid-cols-[1fr_390px]">
        <div>
          <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="grid gap-3 md:grid-cols-[1fr_auto]">
              <input [ngModel]="search()" (ngModelChange)="search.set($event)" [placeholder]="catalogMode() === 'products' ? 'Buscar producto o SKU...' : 'Buscar servicio...'" class="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-teal-500">
            ...
            <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              @if (catalogMode() === 'products') {
                @for (product of filteredProducts(); track product.id) {
                  <article class="catalog-card"><div class="flex items-start justify-between gap-3"><span class="item-icon bg-teal-50 text-teal-700">{{ product.nombre.charAt(0) }}</span><span class="stock-badge" [class.low-stock]="product.stock_actual <= 5">Stock {{ product.stock_actual }}</span></div><p class="item-code">{{ product.sku }} · {{ product.categoria.nombre }}</p><h3 class="item-name">{{ product.nombre }}</h3><div class="mt-4 flex items-center justify-between"><b class="text-xl text-teal-700">S/ {{ money(product.precio_venta) }}</b><button (click)="addProduct(product)" [disabled]="product.stock_actual === 0 || !registerOpen()" class="add-button bg-[#102a43] hover:bg-teal-600">+</button></div></article>
        ...
        <aside class="h-fit rounded-3xl border border-slate-200 bg-white shadow-lg xl:sticky xl:top-24">
          <div class="border-b border-slate-100 p-5"><div class="flex items-center justify-between"><h3 class="text-xl font-black text-[#102a43]">Venta actual</h3><span class="rounded-full bg-teal-50 px-3 py-1 text-xs font-bold text-teal-700">{{ cartCount() }} items</span></div></div>
          ...
          <div class="space-y-5 border-t border-slate-100 p-5">
            ...
            <div class="rounded-2xl bg-[#102a43] p-4 text-white"><div class="flex justify-between text-sm text-slate-300"><span>Subtotal</span><span>S/ {{ subtotalWithoutTax() }}</span></div><div class="mt-2 flex justify-between text-sm text-slate-300"><span>IGV (18%)</span><span>S/ {{ tax() }}</span></div><div class="mt-3 flex justify-between border-t border-white/15 pt-3 text-xl font-black"><span>Total</span><span>S/ {{ total() }}</span></div></div>
            <button (click)="charge()" [disabled]="!canCharge() || charging()" class="w-full rounded-xl bg-[#f4c430] px-5 py-3.5 font-black text-[#102a43] hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-40">{{ charging() ? 'Procesando...' : 'Cobrar y generar boleta' }}</button>
          </div>
        </aside>
      </div>
    </section>
  ...
  styles: [`
    ...
    .mode-active { background: white; color: #0f766e; box-shadow: 0 1px 3px rgb(15 23 42 / .12); }
    ...
    .category-active { border-color: #0d9488; background: #f0fdfa; color: #0f766e; }
    ...
    .catalog-card:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgb(15 23 42 / .08); }
    ...
    .item-name { margin-top: .5rem; min-height: 3rem; color: #102a43; font-weight: 900; line-height: 1.5rem; }
    ...
    .payment-active { border-color: #14b8a6; background: #f0fdfa; color: #115e59; }
  `],
})
